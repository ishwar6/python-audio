...
Next, you'll need to create a MySQL database where AudioRecognizer can store fingerprints. For example, on your local setup:
	
	$ mysql -u root -p
	Enter password: **********
	mysql> CREATE DATABASE IF NOT EXISTS AudioRecognizer;
...
Start by creating a AudioRecognizer object with your configurations settings (AudioRecognizer takes an ordinary Python dictionary for the settings).
...
```python
>>> arc.fingerprint_directory("va_us_top_40/mp3", [".mp3"], 3)
```

For a large amount of files, this will take a while. However, AudioRecognizeru is robust enough you can kill and restart without affecting progress: AudioRecognizeru remembers which songs it fingerprinted and converted and which it didn't, and so won't repeat itself. 

You'll have a lot of fingerprints once it completes a large folder of mp3s:
```python
>>> print arc.db.get_num_fingerprints()
5442376
```
...
The configuration object to the AudioRecognizeru constructor must be a dictionary. 
...
* `fingerprint_limit`: allows you to control how many seconds of each audio file to fingerprint. Leaving out this key, or alternatively using `-1` and `None` will cause AudioRecognizeru to fingerprint the entire audio file. Default value is `None`.
...
There are two ways to recognize audio using AudioRecognizer. You can recognize by reading and processing files on disk, or through your computer's microphone.
...
```bash
$ python audio_recognizer.py --recognize file sometrack.wav 
...
or in scripting, assuming you've already instantiated a audio_recognizer object: 

```python
>>> from audio_recognizer.logic.recognizer.file_recognizer import FileRecognizer
>>> song = arc.recognize(FileRecognizer, "va_us_top_40/wav/Mirrors - Justin Timberlake.wav")
```
...
>>> song = arc.recognize(MicrophoneRecognizer, seconds=10) # Defaults to 10 seconds.
```

and with the command line script, you specify the number of seconds to listen:

```bash
$ python audio_recognizer.py --recognize mic 10
```
...
Test your AudioRecognizer settings on a corpus of audio files on a number of different metrics:
...
An example script is given in `test_audio_recognizer.sh`, shown below:

```bash
#####################################
### AudioRecognizer example testing script ###
#####################################
...
python audio_recognizer.py --fingerprint ./mp3/ mp3
...
# store results in ./results and log to ./results/audio_recognizer-test.log
python run_tests.py \
    --secs 5 \
    --temp ./temp_audio \
    --log-file ./results/audio_recognizer-test.log \
    --padding 8 \
    --seed 42 \
    --results ./results \
    ./mp3
...
For a more detailed look at the making of AudioRecognizer, see my blog post [here](https://willdrevo.com/fingerprinting-and-audio-recognition-with-python/).
...
To test AudioRecognizer's speed and accuracy, I fingerprinted a list of 45 songs from the US VA Top 40 from July 2013 (I know, their counting is off somewhere). I tested in three ways:

1. Reading from disk the raw mp3 -> wav data, and
1. Playing the song over the speakers with AudioRecognizer listening on the laptop microphone.
...
Reading from disk was an overwhelming 100% recall - no mistakes were made over the 45 songs I fingerprinted. Since AudioRecognizer gets all of the samples from the song (without noise), it would be nasty surprise if reading the same file from disk didn't work every time!

### 2. Audio over laptop microphone

Here I wrote a script to randomly chose `n` seconds of audio from the original mp3 file to play and have AudioRecognizer listen over the microphone. To be fair I only allowed segments of audio that were more than 10 seconds from the starting/ending of the track to avoid listening to silence. 
...
Even with only a single second, randomly chosen from anywhere in the song, AudioRecognizer is getting 60%! One extra second to 2 seconds get us to around 96%, while getting perfect only took 5 seconds or more. Honestly when I was testing this myself, I found AudioRecognizer beat me - listening to only 1-2 seconds of a song out of context to identify is pretty hard. I had even been listening to these same songs for two days straight while debugging...

In conclusion, AudioRecognizer works amazingly well, even with next to nothing to work with. 

### 3. Compressed streamed music played on my iPhone

Just to try it out, I tried playing music from my Spotify account (160 kbit/s compressed) through my iPhone's speakers with AudioRecognizer again listening on my MacBook mic. I saw no degredation in performance; 1-2 seconds was enough to recognize any of the songs.
